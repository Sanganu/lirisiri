...
function getmoviereview(mvname) {
  ...
  var queryurl = "http://www.omdbapi.com/?s=" + mvname + "&plot=short&apikey=" + process.env.OMDB_API;
  ...
  omdb(queryurl, function (error, response, body) {
    ...
    if (!error && response.statusCode === 200) {
      let apiresp = JSON.parse(body).Search
      // console.log(apiresp)
     
      console.log("----------------------------------------------------")
      for (let i = 0; i < apiresp.length; i++) {
   
        console.log('Title: ' + apiresp[i].Title);
        console.log('Year: ' + apiresp[i].Year);
        console.log('Id: ' + apiresp[i].imdbID);
        console.log('Poster URL: ' + apiresp[i].Poster);
        console.log("----------------------------------------------------")
      }
